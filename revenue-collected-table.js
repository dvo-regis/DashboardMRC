document.addEventListener("DOMContentLoaded", () => {
    ...
    const tbodyReceived = document.querySelector("#revenueCollectedTable tbody");
  
    // Function to create table rows
    revenueCollectedData.forEach(rowData => {
    ...
    function calculateTotals() {
      const totals = Array(revenueCollectedData[0].length - 2).fill(0);
  
      revenueCollectedData.forEach(row => {
        row.slice(2).forEach((cell, index) => {
          totals[index] += parseFloat(cell);
        });
      });
  
      document.getElementById("revenueCollectedTotalTotal").textContent = totals[0].toFixed(2);
      document.getElementById("revenueCollectedTotal2023").textContent = totals[1].toFixed(2);
      document.getElementById("revenueCollectedTotal2022").textContent = totals[2].toFixed(2);
      document.getElementById("revenueCollectedTotal2021").textContent = totals[3].toFixed(2);
      document.getElementById("revenueCollectedTotal2020").textContent = totals[4].toFixed(2);
      document.getElementById("revenueCollectedTotal2019").textContent = totals[5].toFixed(2);
      document.getElementById("revenueCollectedTotal2018").textContent = totals[6].toFixed(2);
      document.getElementById("revenueCollectedTotal2017").textContent = totals[7].toFixed(2);
    }
