document.addEventListener("DOMContentLoaded", () => {
    ...
    const tbodyReceived = document.querySelector("#revenueBilledTable tbody");
  
    // Function to create table rows
    revenueBilledData.forEach(rowData => {
    ...
    function calculateTotals() {
      const totals = Array(revenueBilledData[0].length - 2).fill(0);
  
      revenueBilledData.forEach(row => {
        row.slice(2).forEach((cell, index) => {
          totals[index] += parseFloat(cell);
        });
      });
  
      document.getElementById("revenueBilledTotalTotal").textContent = totals[0].toFixed(2);
      document.getElementById("revenueBilledTotal2023").textContent = totals[1].toFixed(2);
      document.getElementById("revenueBilledTotal2022").textContent = totals[2].toFixed(2);
      document.getElementById("revenueBilledTotal2021").textContent = totals[3].toFixed(2);
      document.getElementById("revenueBilledTotal2020").textContent = totals[4].toFixed(2);
      document.getElementById("revenueBilledTotal2019").textContent = totals[5].toFixed(2);
      document.getElementById("revenueBilledTotal2018").textContent = totals[6].toFixed(2);
      document.getElementById("revenueBilledTotal2017").textContent = totals[7].toFixed(2);
    }
